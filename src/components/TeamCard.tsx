...
export const TeamCard = ({ team, rank, onEdit, onViewDetail, isAdmin }: TeamCardProps) => {
  ...
  return (
    <div
      className={cn(
        "bg-card rounded-xl p-4 shadow-card transition-all duration-300 hover:shadow-elevated hover:-translate-y-0.5 animate-slide-up group cursor-pointer",
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          <div
            className={cn(
              "w-10 h-10 rounded-lg flex items-center justify-center font-bold text-lg shrink-0 border",
              getRankStyle()
            )}
          >
            {rank}
                      ...
                      {tierCountries.map((country) => (
                        <Tooltip key={country.code}>
                          <TooltipTrigger asChild>
                            <span className="text-sm transition-transform group-hover:scale-110 cursor-default">
                              {country.flag}
                            </span>
                          </TooltipTrigger>
